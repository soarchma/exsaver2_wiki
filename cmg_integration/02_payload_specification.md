...
### 1.1.5. 전역 가변 배열 크기 제한 및 보호 정책 (Global Array Hard Limits)

임베디드 장비(CMG)의 메모리 부족(OOM) 방지 및 네트워크 트래픽 스파이크(Thundering Herd)를 차단하기 위해, 시스템 내 모든 가변 배열은 아래의 하드 리밋(Hard Limit)을 엄격히 준수해야 합니다.

1. **`SCAN_UNPROVISIONED` (미할당 스캔)**:
   - `unprovisioned_devices` 배열은 **최대 50개**까지만 포함합니다.
   - 초과 시 응답 데이터에 `"has_more": true` 플래그를 추가하여 서버에 잔여 기기가 있음을 알립니다.
2. **`SYNC_DIGEST` (무결성 해시 보고)**:
   - 부팅 지연 방지를 위해 CMG가 보관 가능한 최대 논리 그룹(`groups`)은 **100개**, 스케줄(`schedules`)은 **100개**로 제한됩니다.
3. **`GROUP_EXECUTION_REPORT` (실패 타겟 보고)**:
   - 다중 제어 실패 시 `failed_targets` 배열은 **최대 30개**까지만 포함합니다.
   - 초과 시 배열을 자르고(Truncate), 최상위에 `"has_more_errors": true` 플래그를 추가하여 서버가 대규모 장애 상황임을 인지하도록 합니다.
4. **`MANAGE_GROUP` / `MANAGE_SCHEDULE` (설정 동기화)**:
   - 서버는 펌웨어 파싱 부하를 막기 위해 하나의 논리 그룹(`targets`)이나 스케줄 액션(`actions`) 배열에 **최대 100개**의 타겟만 포함하여 전송합니다.
...
- `ON`: 릴레이 즉시 켬 + 대기전력 차단 모니터링 비활성화
- `OFF`: 릴레이 즉시 끔 + 대기전력 차단 모니터링 비활성화
- `STANDBY`: 릴레이 즉시 켬 + 대기전력 차단 모니터링 활성화

> 💡 **[중요] STANDBY 우아한 우회 (Graceful Bypass) 정책**
> 특정 채널의 대기전력 차단 기준값(`threshold_w`)이나 지연 시간(`delay_sec`)이 `0`으로 미설정된 상태에서 `STANDBY` 명령이 하달될 수 있습니다.
> 이 경우 펌웨어는 대규모 스케줄 제어 시 에러 폭주를 막기 위해 **실패(FAILED)를 발생시키지 않고 정상 완료(`status: "COMPLETED"`)로 응답**해야 합니다. 단, 내부적으로는 전력량 비교 및 차단 로직을 실행하지 않고 스킵(Bypass)하여 릴레이를 유지합니다.
...
기존에 `lcu_id` 객체를 반복 전송하던 방식에서, **동일 기기 내 다중 채널을 정수형 배열(`ch_no: [1, 2]`)로 압축**하여 JSON 덩치를 절반 이하로 줄인 최종 규격입니다.
...
// [요청] Server ➔ CMG
{
  ...
  "target_id": "lcu_001122334402",
  "channels": [
    { "action": "OFF", "ch_no": [1, 2] }, // 💡 동일 명령을 수행할 채널을 배열로 압축
    { "action": "STANDBY", "ch_no": [3] }
  ]
}
...
네트워크 패킷 증발로 인한 프론트엔드 무한 로딩을 방지하기 위해, **예외적으로 `data` 토픽(QoS 0)으로 우회하지 않고 `cmd_ack` 응답 페이로드(QoS 1)에 계측 데이터를 직접 담아 회신**합니다.

> 

**💡 [참고: 데이터 형식 예외 적용]**
`POLL_DATA` 명령의 응답은 실시간 대시보드 렌더링 최적화 및 기존 수집 파이프라인과의 호환성을 위해, 예외적으로 제어 응답의 Full Word 원칙을 따르지 않고 Telemetry 데이터의 약어(Short Key) 규격을 그대로 포함하여 회신합니다.
...
// [응답] CMG ➔ Server (QoS 1 보장)
{
  ...
  "data": {
    ...
    "h": 35.2,
    "ch": [
      { "n": 1, "s": 1, "w": 150.5, "a": 0.68, "pf": 0.95, "sw": 0.0, "cw": 10500.5, "cs": 120.0 }
      // ... 하위 채널 데이터 포함
    ]
  },
  "message": "Immediate telemetry polled"
}
...
// [1차 응답 - 수신 직후 발행] CMG ➔ Server
{
  ...
  "target_id": "cmg_001122334401",
  "status": "IN_PROGRESS",
  "message": "Scanning unprovisioned devices..."
}
...
// [최종 응답] CMG ➔ Server
{
  ...
  "data": {
    ...
    "has_more": true // 💡 기기 개수가 50개를 초과하여 잔여 기기가 있을 경우 플래그 추가
  }
}
```
...
// [요청] Server ➔ CMG
{
  ...
  "target_id": "lcu_001122334402",
  "channels": [
    { "ch_no": [1, 2, 3], "mode": "NORMAL", "threshold_w": 5.0, "delay_sec": 300 }, // 💡 3개 채널 동시 설정
    { "ch_no": [4], "mode": "FORCE_ON", "threshold_w": 0.0, "delay_sec": 0 }
  ]
}
...
// [요청] Server ➔ CMG : 스케줄 생성 또는 수정 (UPSERT)
{
  ...
  "schedule_data": {
    ...
    "actions": [
      ...
      { "target_type": "DEVICE", "lcu_id": "lcu_001122334405", "ch_no": [4, 5, 6], "action": "OFF" }
    ]
  }
}
...
```json
{
  ...
  "failed_targets": [
    // 💡 통신 단절(OFFLINE) 기기의 경우 4~6채널이 전부 실패하므로 배열로 압축하여 보고
    {
      "lcu_id": "lcu_001122334455",
      "ch_no": [1, 2, 3, 4],
      "reason": "OFFLINE"
    },
    { "lcu_id": "lcu_001122334456", "ch_no": [1], "reason": "RELAY_FAULT" }
  ],
  "has_more_errors": true // 💡 실패 타겟이 30개를 초과할 경우 배열을 자르고 플래그 추가
}
```
